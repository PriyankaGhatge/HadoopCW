...
public class ReduceJoinEx {
	...
	public static class CustsMapper extends Mapper<LongWritable,Text,Text,Text>
	{
		public void map(LongWritable key, Text value, Context con) throws IOException, InterruptedException {
			...
			con.write(new Text(parts[0]), new Text("custs\t"+parts[1]));
	...
	public static class TxnsMapper extends Mapper<LongWritable,Text,Text,Text>
	{
		public void map(LongWritable key, Text value, Context con) throws IOException, InterruptedException {
			String record = value.toString();
			String [] parts= record.split(",");
			con.write(new Text(parts[2]), new Text("txns\t"+parts[3]));
	 ...
	 public static class ReduceJoinReducer extends Reducer<Text,Text,Text,Text>
	   {
		 ...
		 public void reduce(Text key, Iterable<Text> values, Context con) throws IOException, InterruptedException 
		 {
			...
			String arr=String.format("%d\t%f",count,total);
			con.write(new Text(name), new Text(arr));
